        ...
        const map = L.map('map').setView([31, -8.5], 8.5); 
...
fetch('epicentre.geojson')  
    .then(response => response.json())
    .then(data => {
        addEpicentreToMap(data); 
    })
    .catch(error => {
...
fetch('hopitaux.geojson')  
    .then(response => response.json())
    .then(data => {
        hospitalData = data;
        populateHospitalSelect(data); 
    })
    .catch(error => {
        console.error("Erreur lors du chargement du fichier GeoJSON des hôpitaux : ", error);
    });
    
    let damagedBuildingsLayer; 
    let currentLayer = null;
let undamagedBuildingsLayer; 
...
function showBuildingsInRadius(radius) {
    ...
    const filteredBuildings = damagedBuildingsLayer.toGeoJSON().features.filter((feature) => {
        const buildingDistance = parseFloat(feature.properties.dis_epicentre_km);  
        return Math.round(buildingDistance) <= radius; 
    });

    
    currentLayer = L.geoJSON(filteredBuildings, {
        style: {
            color: 'blue', 
            fillOpacity: 1
        },
...
document.getElementById("showBuildingsBtn").addEventListener("click", () => {
    const radiusInput = document.getElementById("radiusInput").value;

    if (radiusInput) {
        const radius = parseInt(radiusInput, 10);
        if (radius > 0) {
            showBuildingsInRadius(radius); 
        } else {
            alert("Veuillez entrer un rayon positif.");
        }
    } else {
        alert("Veuillez entrer un rayon.");
    }
});
...
function populateHospitalSelect(data) {
    ...
    data.features.forEach(feature => {
        const hospitalName = feature.properties.nom; 
...
function populateCommuneSelect(data) {
    ...
    data.features.forEach(feature => {
        const communeName = feature.properties.nom_commun; 
        const option = document.createElement('option');
